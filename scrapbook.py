    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                # Parse dates if they exist - handle multiple date formats
                assigned_date = None
                acquired_date = None
                expires_date = None
                
                def parse_date_safely(date_str, row_num, field_name):
                    """Parse date string with multiple format attempts"""
                    if not date_str or not str(date_str).strip() or str(date_str).strip().lower() in ['nan', 'none', '']:
                        return None
                    ...
                    print(f"Row {row_num}: Could not parse {field_name} date: '{date_str}'")
                    return None
                
                assigned_date = parse_date_safely(assigned_on, index + 1, "assigned_on")
                acquired_date = parse_date_safely(acquired_on, index + 1, "acquired_on")
                expires_date = parse_date_safely(expires_on, index + 1, "expires_on")
...
def insert_employee_compliance_status_bulk(csv_file_path, database_url):
    ...
    try:
        ...
        for data in valid_data:
            # Parse dates
            assigned_date = parse_date_safely(data['assigned_on'], data['row_index'], "assigned_on")
            acquired_date = parse_date_safely(data['acquired_on'], data['row_index'], "acquired_on")
            expires_date = parse_date_safely(data['expires_on'], data['row_index'], "expires_on")
